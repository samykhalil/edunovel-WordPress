...
?>
	<style>
		...
		.myaccount .heading .name {
			font-size: 25px;
			display: flex;
			gap: 10px;
		}

		.myaccount .heading .name img {
			border-radius: 50%;
			height: fit-content
		}
		...
		.myaccount .ordersContent {
			display: none;
		}

		.myaccount .ordersContent h6 {
			text-align: center;
			margin: 50px 0;
		}

		.myaccount .woocommerce-orders-table__row {
			display: flex;
			justify-content: space-between;
			margin-bottom: 10px;
		}

		.myaccount .woocommerce-orders-table__row .woocommerce-orders-table__cell {
			width: 33.333%;
			text-align: center;
		}
		...
		.myaccount .woocommerce-orders-table__cell-order-actions a {
			margin: 0 5px
		}
			...
			$(document).ready(function() {
				...
				$(".prefix-cart-notes").on("change", function() {
					...
					if (value != '' && !emailReg.test(value)) {
						$(this).after('<p class="email_error">برجاء ادخال صيغة بريد إلكتروني صحيحة</p>')
					} else {
						self.parent().append('<div class="spinner-border" role="status"></div>')
						$(".cart_totals").block({
							message: null,
							overlayCSS: {
								background: "#fff",
						...
						$.ajax({
							...
							success: function(response) {
								if (response['success']) {
									...
									$('.spinner-border').remove();
									self.after('<p class="email_error">البريد الإلكتروني غير مسجل لدينا</p>')
								}
							},
						});
				...
				$('.assignTo button').on('click', function(e) {
					...
					// inputContainer
					parent.find('.inputContainer').append('<div class="spinner-border" role="status"></div>')
					$.ajax({
						type: "POST",
						url: prefix_vars.ajaxurl,
						data: {
				...
				$('form#student-registration').on('submit', function(e) {
					...
					$.ajax({
						...
						success: function(data) {
							...
							// }
							if (!data['success']) {
								$('.status').append("<p class='error'>" + data['message'] + "</p>")

							} else {
								$('.status').append("<p class='success'>تم اضافة الطالب بنجاح</p>")
								setTimeout(() => {
									location.reload();
								}, 300);
							}
				...
				$('.showorders').on('click', function() {
					let ordersContent = $(this).parent().parent().next().find('.ordersContent')
					let changepassword = $(this).parent().parent().next().find('.changepassword')
					...
					$(this).parent().parent().siblings(".userContent").find('.changepassword').hide()
					$(this).parent().parent().siblings(".userContent").find('.ordersContent').hide()
					changepassword.hide()
					ordersContent.show()
					//    }
				});
				...
				$('.reBuy').on('click', function(e) {
					...
					$.ajax({
						...
						success: function(data) {
							$('.status').append("<p class='success'>" + data['message'] + "</p>")
							setTimeout(() => {
								location.reload();
							}, 300);
				...
				$('.countdown-item').each(function() {
					...
					function updateCountdown() {
						...
						if (timeDifference <= 0) {
							...
							countdownElement.text(days + ' يوم و  ' + hours + ' ساعة و  ' + minutes + ' دقيقة ' + seconds + ' ثانية');
						}
					}
